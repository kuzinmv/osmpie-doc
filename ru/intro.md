...
OSMPIE - это больше, чем рендер-движок площадных объектов дороги. В его основе лежит построение точной топологической и объектной модели большинства элементов дорожной сети. Он обобщает множество точек в логике перекрестка, создавая объекты (точки входа и выхода из перекрестка, конфликтные точки, подходы), необходимые для инженерных расчетов на перекрестке и управления дорожным движением.

**Продвинутая навигация**

Расширенное объектное моделирование и улучшенное качество визуализации обеспечивают более точную навигацию на дорогах, поскольку предоставляют не только базовые данные о количестве полос и дорожных знаках, но и исчерпывающую информацию о моделях поворотов, геометрии перекрестков, дорожной разметке и других подробных элементах инфраструктуры.

**Обогащение OSM**

Объекты, полученные в результате рендеринга OSMPIE (например, полигоны дорог, area:highway, area:highway=* + junction=yes, знаки разметки, уточненные теги поворотов turn:lanes для way[highway], полигоны и разметка пешеходных переходов и многое другое) можно сохранять в OSM.
...
Подробнее о всех тэгах можео узнать в [глоссарии](./osmpie.tags.glossary.md)
