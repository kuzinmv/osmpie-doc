...
Существующая система описания пересечений в OpenStreetMap характеризуется фрагментарностью и отсутствием единого методологического подхода. Базовые теги `junction=yes` и `junction=uncontrolled`, определённые в [Key:junction](https://wiki.openstreetmap.org/wiki/Key:junction), обеспечивают лишь элементарную функциональность и не позволяют детально классифицировать сложные транспортные узлы. В результате перекреток может выглядеть на карте вполне органично, но он топологически не достоверен. При картировании "официальными" методами OSM теряется важная информация, необходимая для решения прикладных задач.
...
В системе OSMPIE применяется модель окружности с параметром радиуса ([cluster:radius](./node.tags.junction:cluster:radius.md)), определяющим зону потенциальных конфликтов участников движения.

#### 3. Топологическая связность
Поддерживается система связей между входными и выходными точками пересечения, определяющая допустимые траектории движения.

**Инструментарий описания**:
- Тег [connect:lanes](./way.tags.connect:lanes.md) — детализированное описание связности полос движения
- Relation `type=connectivity` — комплексное описание маршрутов через пересечение

#### 4. Конфликтные зоны
Зона конфликта — это область пересечения траекторий различных участников движения с указанием координат потенциальных точек конфликта.

### Кластеризация пересечений

Множественные пересечения склонны к образованию сложных топологических структур — перекрёстков. Процесс кластеризации регулируется параметром [junction:cluster:radius](./node.tags.junction:cluster:radius.md), определяющим максимальное расстояние между пересечениями для их объединения в единый перекрёсток.
...
#### junction = controlled
Пересечение, на котором дорожное движение регулируется активными системами.
