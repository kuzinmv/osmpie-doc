...
Для создания идеального [перекрёстка](./node.tags.junction.md) в OSMPIE нам потребуются следующие ингредиенты:
...
- [Типы пересечений и новые теги junction:*](#4-пересечения-и-перекрёстки)
...
- [Дорожная разметка (в разработке)](#7-разметка)
...
**Важно:**  
- Если теги не указаны, рендер по умолчанию использует две полосы и двустороннее движение.  
...
Рассмотрим основные теги для управления поворотами и соединениями полос.

Ключ [turn](https://wiki.openstreetmap.org/wiki/Key:turn) является одним из наиболее используемых ингредиентов 
хорошо приготовленного перекрёстка (или развязки), поэтому настоятельно рекомендуем ознакомиться со статьёй.
...
В качестве альтернативы, позволяющей с удобством задавать сложные соединения на перекрёстке, 
мы предлагаем вместо отношения использовать ключ `connect:lanes`, о чём можно прочесть в дополнительной статье [connect:lanes](./way.tags.connect:lanes.md).  
...
Тег `width` задаёт полную [ширину улицы](https://wiki.openstreetmap.org/wiki/Key:width#Width_of_streets).
Остальные теги, описываемые в статье, применяются в OSMPIE, чтобы задать ширину других объектов дороги.
...
Используемый в OSM тег `placement` является чуть ли не единственным средством, которое позволяет исправить ошибки, 
создаваемые осевыми линиями на дорогах с изменяющимся количеством полос и на развилках. Данный тег незаменим при маппинге полос.

**Советуем** внимательно ознакомиться с положениями и применениями крайне важного тега [placement](https://wiki.openstreetmap.org/wiki/Proposal:Placement), 
особенно со значением `placement:transition`.  

Обращаем внимание, что этот тег адресуется к краям или центру какой-либо полосы, что сильно ограничивает его применение для точного маппинга.
...
- . вводя суффиксы forward и backward и учитывая их отдельно друг от друга, мы получаем раздвоение осевой
...
для коротких way, чтобы отразить левые повороты на перекрёстках, где автомобили расходятся правыми бортами. 
...
[key:junction](https://wiki.openstreetmap.org/wiki/Key:junction). Чтобы готовить перекрёстки
...
Сейчас в OSM используются теги `junction:yes`, `junction:uncontrolled`.
...
* `junction:uncontrolled` - не контролируемое пересечение/перекрёсток так как это используется сейчас
...
В большинстве случаев рендер автоматически вычисляет эти теги по другим ключам и тегам участвующих в соединениях объектах.
В таких случаях эти теги не требуются прописывать или изменять. В редких случаях, например, когда в регулируемом пересечении участвуют выходы из парковок и дворов
`way[highway:service]`, перечисленные теги следует задать явно, 
...
1. `junction:shape` - определяет форму пересечения (прямоугольник, параллелограмм или ступеньки); если явно не задана, то osmpie 
...
В случаях пересечения 5 ways и более необходимо задавать теги явно и вручную.
3. `junction:radius` - радиус окружности, соотносящийся с зоной конфликта, в котором участвуют транспортные средства на данном перекрёстке.
...
5. `junction:cluster:radius` - минимальный радиус окружности, объединяющий точки пересечений ways в один кластер пересечений, что соответствует "настоящему перекрёстку" в обычном понимании.
6. `crossing:corner:yes|no` - тег для точек пешеходных переходов. Оформляется наподобие тега `crossing:island`, как описано в статье
   [crossing:island](https://wiki.openstreetmap.org/wiki/Key:crossing:island), который 
указывает на наличие островка безопасности. Тег `crossing:corner` отмечает факт, что пешеходный переход находится 
очень близко к проезжей части справа или слева. То есть фактически проведён от угла одного тротуара 
до угла тротуара противоположной стороны way. Учёт такого пешеходного перехода позволяет строить топологически правильные формы пересечения.
...
В первой версии OSMPIE реализована частичная поддержка тегов парковки и велодорожек.
Учитываются только те параметры, которые влияют на ширину проезжей части и полигоны дороги, то есть расположение и ширина.
...
в кармане. Таким образом возникает задача отрисовать этот карман и разметку в нём.

Для остановок в которых указан тег `tram=yes` применяется иной вид разметки - перпендикулярный полосам движения
...
Отдельно стоит упомянуть что пересечения автомобильных путей с трамвайными путями и велодорожками игнорируются, если они не помечены подходящими тегами,
например `highway = traffic_signals`. Отсутствие тегов означает, что здесь не требуется решение связности этих пересечений. Учёт этих пересечений без определения
связанности и отношений порождает множество мелких рёбер, что искажает геометрическую картину полос на автомобильной дороге.  
...
В OSMPIE существует набор дополнительных тегов, с помощью которых можно управлять рендером. 
...
osmpie разделяет рендер полигонов разных уровней дороги для случаев тоннелей и развязок с перекрёстками на других уровнях.
В редких случаях перекрёсток может быть частью моста, либо мост может находиться внутри перекрёстка. А таких конструкциях OSM не позволяет построить полигон перекрёстка,
а изменить тег level для моста противоречит правилам OSM.
...
Этот тег явным образом указывает, надо ли проводить процедуру удаления для конкретного way и, если надо, то с какими ограничениями.

* `osmpie:fill = yes|no|number` - В osmpie работает процедура наполнения way точками, если way содержит длинные участки без точек. 
Этот тег явным образом указывает, надо ли проводить процедуру наполнения для конкретного way и, если надо, то с какими ограничениями.
...
Этот тег обозначает, что конкретный way не надо удалять.


[Примеры использования этих тегов в OSMPIE](./examples/fill.sparse.md)
...
В первой публичной версии osmpie разметка не управляема и генерируется автоматически. Поэтому она может не отражать некоторых местных особенностей нанесения,
...
конкретных объектов (например, полосы) и её размерах. Одна из первоочередных задач — открыть возможность редактирования разметки.

Типичными тегами, указывающими на дорожную разметку, являются `divider`, `turn:lanes=*`, `overtaking=*`, `change=*` или `crossing:markings=*.` 
Наличие разметки на дороге можно указать с помощью тега `lane_markings=*` 
Использование вышеупомянутых тегов в сочетании с дополнительной информацией, такой как `placement=*` или `width:lanes=*`,
...
Данный принцип редактирования разметки поддерживается OSMPIE за исключением того, что знаки поворотов и текст, изображённые на полосе, должны быть обозначены как way объекты.
